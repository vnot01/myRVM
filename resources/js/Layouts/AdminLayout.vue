...
watch(() => page.props.flash, (newFlashValue, oldFlashValue) => {
  ...
  //     let message = newFlashValue.success;
  //     if (message.length > MAX_FLASH_LENGTH) {
  //         message = message.substring(0, MAX_FLASH_LENGTH) + '...';
  //     }
  //     flashMessageText.value = message;
...
//     if (flash.success) {
//         flashMessageText.value = flash.success;
//         flashMessageType.value = 'success';
//     } else if (flash.error) {
//         flashMessageText.value = flash.error;
//         flashMessageType.value = 'error';
//     } // Tambahkan untuk warning/info jika perlu
//     showFlashMessage.value = true;
//     console.log('showFlashMessage di-set:', showFlashMessage.value, 'Teks:', flashMessageText.value, 'Tipe:', flashMessageType.value); // DEBUG
//     setTimeout(() => {
//       showFlashMessage.value = false;
//     }, 5000);
//     // showFlashMessage.value = true;
...
<template>
    <div>
        <Head :title="title" />
        <!-- {/* --- AWAL PESAN NOTIF / FLASH MELAYANG --- */} -->
        <!-- {/* --- PESAN FLASH SEDERHANA MELAYANG --- */} -->
         <!-- /* Muncul dari kanan */ -->
          <!-- /* Hilang ke kanan */ -->
        <transition
            ...
            {/* sm:translate-x-0 adalah untuk memastikan di layar sm ke atas ia tidak tergeser (jika ada logika mobile transform) */} -->
                ...
                <nav class="flex-grow p-4 space-y-1">
                    ...
                    <NavLink :href="route('admin.rvms.index')" :active="page.url.startsWith('/admin/rvms')" v-if="page.props.auth.user.role === 'Admin'">
                        <svg class="mr-3 w-[27px] h-[27px] text-gray-400 dark:text-gray-400 group-hover:text-gray-900 dark:group-hover:text-gray-300" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" viewBox="0 0 24 24">
                    ...
                    <NavLink :href="route('admin.users.index')" :active="page.url.startsWith('/admin/users')" v-if="page.props.auth.user.role === 'Admin'">
                         <svg class="mr-5 w-5 h-5 text-gray-400 transition duration-75 dark:text-gray-400 group-hover:text-gray-900 dark:group-hover:text-gray-300"
